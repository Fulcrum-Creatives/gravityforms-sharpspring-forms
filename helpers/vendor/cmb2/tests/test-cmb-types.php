...
class Test_CMB2_Types extends Test_CMB2 {
	...
	public function test_get_file_ext() {
		...
		$ext = $type->get_file_ext( site_url( '/wp-content/uploads/2016/12/test-file.pdf' ) );
		$this->assertEquals( 'pdf', $ext );
	}

	public function test_get_file_name_from_path() {
		$type = $this->get_field_type_object( 'file' );
		$name = $type->get_file_name_from_path( site_url( '/wp-content/uploads/2016/12/test-file.pdf' ) );
		$this->assertEquals( 'test-file.pdf', $name );
	}

	public function test_is_valid_img_ext() {
		$type = $this->get_field_type_object( 'file' );
		$this->assertFalse( $type->is_valid_img_ext( $type->get_file_ext( site_url( '/wp-content/uploads/2016/12/test-file.pdf' ) ) ) );
	...
	public function test_text_money_field() {
		 ...
		 * but use a British pound symbol for the gfssf
		 */
